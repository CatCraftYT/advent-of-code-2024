...
type Line = (Integer, Integer, Integer) -- Ax+Bx=C
...
correctCoords :: [ClawMachine] -> [ClawMachine]
correctCoords = map (bimap correctFunc correctFunc)
    where correctFunc (a,b,c) = (a,b,c+10000000000000)

main = do
    contents <- splitOn "\n\n" <$> readFile "inputs/day13.txt"
    print $ sum.map (\p -> 3 * fst p + snd p) . clawMachineIntersections.correctCoords.parseInput $ contents
