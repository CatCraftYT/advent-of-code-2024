...
createListCombos :: [Int] -> [[Int]]
createListCombos xs = xs : [[x | (x,n) <- zip xs [1..], n /= i] | i <- [1..length xs]]

main = do
    contents <- readFile "inputs/day2.txt"
    print.length $ filter id [any isSafe (createListCombos.parseLine $ line) | line <- lines contents]
